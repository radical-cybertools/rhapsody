...
async def main() -> None:
    ...
    for name, spec in specs.items():
        print(
            f"partition {name}: {_hostlist_str(spec['nodelist'])} (env keys: {sorted(spec['env'])})"
        )
    ...
    try:
        ...
        print()
        print(
            f"{'partition':<6} {'expected':<24} {'actual':<24} {'pid':<8} {'cores':<22} ROCR / HIP"
        )
        print("-" * 130)
        ...
        for t in tasks:
            ...
            if not isinstance(rv, dict):
                print(
                    f"FAILED {t['uid']:<14} state={t['state']!s:<10} "
            ...
            print(
                f"{rv['partition']:<6} {rv['expected_host']:<24} "
