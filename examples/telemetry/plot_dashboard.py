...
NODE_PALETTE = [
    "#ee5a24", "#2e86de", "#009432", "#f9ca24",
    "#8854d0", "#00b894", "#e17055", "#2d3436",
]
GPU_PALETTE = [
    "#fd79a8", "#a29bfe", "#55efc4", "#fdcb6e",
    "#e17055", "#6c5ce7", "#00cec9", "#ffeaa7",
]
...
def _vline(ax, val, color, label):
    ax.axvline(val, color=color, linestyle="--", linewidth=1.6,
...
def _draw_conc(ax, t_c, y_c):
    ...
    if not t_c:
        return
    ax.step(t_c, y_c, color=C["concurrency"], linewidth=2.5, where="post",
            label="Tasks running", zorder=5)
...
def main(path: str):
    ...
    gs = gridspec.GridSpec(
        4, 4, figure=fig,
        hspace=0.82, wspace=0.44,
        top=0.973, bottom=0.063,
    ...
    if durations_valid or durations_incomplete:
        ...
        ax.hist(
            ...
            bins=nb, stacked=True,
            color=[C["duration"], C["incomplete"]],
            edgecolor="#f4f6f9", linewidth=0.6,
    ...
    if times_completed:
        ax.step(times_completed, cum_completed,
                color=C["cumulative"], linewidth=2.0, where="post", zorder=3)
        ax.fill_between(times_completed, cum_completed,
                        step="post", alpha=0.18, color=C["cumulative"], zorder=2)
    ...
    if len(times_completed) > 1:
        ...
        if dur > 0:
            ...
            ax.bar(centers, counts / bin_w, width=bin_w * 0.85,
                   color=C["throughput"], edgecolor="#f4f6f9", linewidth=0.6, zorder=3)
    ...
    if has_gpu:
        ...
        for node in nodes:
            ...
            for gid in gpu_ids:
                ...
                ax.plot(
                    ...
                    linewidth=2.0, marker="^", markersize=3,
                    label=f"{node.split('.')[0]} G{gid}", zorder=3,
                )
                cidx += 1
        ax.set_ylim(0, 110)
        ...
        ax.text(0.5, 0.5, "No GPU data", ha="center", va="center",
                transform=ax.transAxes, fontsize=13, color="#b2bec3")
        ax.set_axis_off()
    ...
    if items:
        ...
        bars = ax.barh(
            labels_e, vals_e,
            color=bar_colors, edgecolor="#f4f6f9", linewidth=0.6,
            height=0.65, zorder=3,
        )
        for bar, v in zip(bars, vals_e):
            ax.text(v + max(vals_e) * 0.012,
    ...
    if nodes and ev["ResourceUpdate"]:
        ...
        if buckets_cpu:
            ...
            ax_agg2.plot(ts, vs, color=AGG_CPU_CLR, linewidth=3.0,
                         linestyle="-", label="Mean CPU %", zorder=4)
        if buckets_mem:
            ts, vs = _mean_series(buckets_mem)
            ax_agg2.plot(ts, vs, color=AGG_MEM_CLR, linewidth=3.0,
                         linestyle="--", label="Mean Mem %", zorder=4)
        if buckets_gpu:
            ts, vs = _mean_series(buckets_gpu)
            ax_agg2.plot(ts, vs, color=AGG_GPU_CLR, linewidth=3.0,
                         linestyle=":", label="Mean GPU %", zorder=4)

    _decorate_wide(ax_agg,
                   "Aggregated Resource Utilization (all nodes)  +  Concurrency",
                   "Time since start (s)", "Active tasks")
    ...
    for k, node in enumerate(nodes):
        ...
        if per_node:
            ...
            ax_det2.plot(ts, [e.get("cpu_percent") or 0 for e in per_node],
                         color=nc, linewidth=2.5, linestyle="-",
                         label=f"{short} CPU%", zorder=4)
            ax_det2.plot(ts, [e.get("memory_percent") or 0 for e in per_node],
                         color=nc, linewidth=2.5, linestyle="--", alpha=0.80,
                         label=f"{short} Mem%", zorder=4)
    ...
    for node in nodes:
        ...
        for gid in gpu_ids:
            ...
            ax_det2.plot(
                ...
                label=f"{short} GPU{gid}%", zorder=4,
            )
            gpu_idx += 1

    _decorate_wide(ax_det,
