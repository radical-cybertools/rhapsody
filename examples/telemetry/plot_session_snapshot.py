...
def main(path: str):
    ...
    if durations_valid or durations_incomplete:
        ...
        if durations_valid:
            ...
            _vline(ax, np.median(durations_valid), C["median"],
    ...
    if durations_valid:
        ...
        for pct, col, lbl in [
            (0.50, "#e74c3c", "p50"), (0.90, "#f39c12", "p90"),
            (0.95, "#9b59b6", "p95"), (0.99, "#7f8c8d", "p99"),
        ]:
    ...
    ax = axes[i]; i += 1
    conc = (
        [(e["event_time"] - t0, +1) for e in ev["TaskStarted"]] +
    ...
    ax = axes[i]; i += 1
    if times_completed:
        ax.step(times_completed, cum_completed,
                color=C["cumulative"], linewidth=2, where="post")
        ax.fill_between(times_completed, cum_completed,
                        step="post", alpha=0.15, color=C["cumulative"])
    ...
    for ts in tl.values():
        if "Queued" in ts and "Started" in ts:
            ...
            wait.append(val)
    if wait:
        ax.hist(wait, bins=smart_bins(wait),
                color=C["wait"], edgecolor="white", linewidth=0.4)
        mu = np.mean(wait)
        _vline(ax, mu, C["mean"], f"mean {mu:.3f}s")
        ax.legend(fontsize=7)
    _style(ax, "Queue Wait\n(TaskQueued → TaskStarted)",
           "Wait time (seconds)", "Task count")
    ...
    for ts in tl.values():
        if "Submitted" in ts and "Queued" in ts:
            ...
            routing.append(val)
    if routing:
        ax.hist(routing, bins=smart_bins(routing),
                color=C["routing"], edgecolor="white", linewidth=0.4)
        mu = np.mean(routing)
        _vline(ax, mu, C["mean"], f"mean {mu:.4f}s")
        ax.legend(fontsize=7)
    _style(ax, "Routing Latency\n(TaskSubmitted → TaskQueued)",
           "Routing time (seconds)", "Task count")
    ...
    for ts in tl.values():
        if "Submitted" in ts and ("Completed" in ts or "Failed" in ts):
            ...
            e2e.append(val)
    if e2e:
        ax.hist(e2e, bins=smart_bins(e2e),
                color=C["e2e"], edgecolor="white", linewidth=0.4)
        mu = np.mean(e2e)
        _vline(ax, mu, C["mean"], f"mean {mu:.3f}s")
        ax.legend(fontsize=7)
    _style(ax, "End-to-End Latency\n(TaskSubmitted → TaskCompleted)",
           "Latency (seconds)", "Task count")
        ...
        for k, node in enumerate(nodes):
            ...
            if ne:
                ax.plot(
                    ...
                    linewidth=1.5, marker="o", markersize=3, label=node,
                )
        ax.set_ylim(0, 105)
        if nodes:
        ...
        for k, node in enumerate(nodes):
            ...
            if ne:
                ax.plot(
                    ...
                    color=NODE_PALETTE[k % len(NODE_PALETTE)],
                    linewidth=1.5, marker="s", markersize=3, label=node,
                )
        ax.set_ylim(0, 105)
        if nodes:
            ax.legend(fontsize=6)
    ...
    if has_gpu:
        ...
        _style(ax, "GPU Utilization (per device)",
               "Time since start (seconds)", "GPU %")
    else:
    ...
    if items:
        ...
        bars = ax.barh(labels_e, vals_e, color=bar_colors,
                       edgecolor="white", linewidth=0.6)
        for bar, v in zip(bars, vals_e):
            ax.text(v + max(vals_e) * 0.01,
    ...
    if len(times_completed) > 1:
        ...
        if dur > 0:
            ...
            ax.bar(centers, counts / bin_w, width=bin_w * 0.9,
                   color=C["throughput"], edgecolor="white", linewidth=0.4)
    _style(ax, "Task Completion Throughput",
           "Time since start (seconds)", "Tasks per second")

    # ── 14–16. OTel metric counters bar chart ─────────────────────────────────
    ax = axes[i]; i += 1
    if metric_lines:
        m      = metric_lines[-1]
        keys   = ["tasks_submitted", "tasks_started", "tasks_completed",
                  "tasks_failed", "tasks_running"]
    ...
    if deltas:
        ...
        t_c, y_c, running = [], [], 0
        for tc, d in deltas:
            t_c += [tc, tc]; y_c += [running, running + d]; running += d
        if t_c:
            t_c.append(t_c[-1]); y_c.append(0)
        ax.step(t_c, y_c, color=C["concurrency"], linewidth=1.5, where="post",
                label="Concurrency", alpha=0.9)
    ...
    for k, node in enumerate(nodes):
        ...
        per_node = sorted(
            [e for e in ev["ResourceUpdate"] if e.get("node_id") == node and e.get("gpu_id") is None],
            key=lambda e: e["event_time"],
        )
        if per_node:
            ts = [e["event_time"] - t0 for e in per_node]
            ax2.plot(ts, [e.get("cpu_percent") or 0 for e in per_node],
                     color=nc, linewidth=1.2, linestyle="-", label=f"{node} CPU%")
            ax2.plot(ts, [e.get("memory_percent") or 0 for e in per_node],
                     color=nc, linewidth=1.2, linestyle="--", alpha=0.7, label=f"{node} Mem%")
        gpu_ids = sorted({e["gpu_id"] for e in ev["ResourceUpdate"]
                          if e.get("node_id") == node and e.get("gpu_id") is not None})
        for gid in gpu_ids:
            gev = sorted(
                [e for e in ev["ResourceUpdate"] if e.get("node_id") == node and e.get("gpu_id") == gid],
                key=lambda e: e["event_time"],
            )
            gc = NODE_PALETTE[(k + 2) % len(NODE_PALETTE)]
            ax2.plot([e["event_time"] - t0 for e in gev],
                     [e.get("gpu_percent") or 0 for e in gev],
                     color=gc, linewidth=1.2, linestyle=":", label=f"{node} GPU{gid}%")
    ax2.set_ylim(0, 105)
