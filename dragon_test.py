...
# Dragon symbols are resolved lazily by _import_dragon_symbols() below.
# Keeping the module import-clean lets DragonExecutionBackendV3Client be
# imported by a user process without pulling `dragon` (and its side effects,
# e.g. multiprocessing monkey-patches) into that process's namespace.
Batch = None
ProcessTemplate = None


def _import_dragon_symbols() -> None:
    """Import Dragon symbols needed by the V3 backend into module globals.

    Idempotent. Called only from code paths that run inside the Dragon head
    process (or a process that is about to become one). The Client path never
    calls this, so user processes stay free of Dragon imports.
    """
    global Batch, ProcessTemplate
    if Batch is not None:
        return
    try:
        import multiprocessing as mp  # noqa: F401 — dragon patches this
        import dragon  # noqa: F401
        from dragon.native.process import ProcessTemplate as _ProcessTemplate
        from dragon.workflows.batch import Batch as _Batch
...
class DragonExecutionBackendV3(BaseBackend):
    ...
    def __init__(
        ...
        # Resolve Dragon symbols lazily — this is the first point where they
        # are actually needed. The Client path never reaches here.
        _import_dragon_symbols()
        if not Batch:
    ...
    async def submit_tasks(self, tasks: list[dict]) -> None:
        ...
        # Build AND register each task in a single pass (§5.2). The Batch
        # background thread starts dispatching as soon as build_task() returns,
        # so if registration is deferred until the whole build loop completes,
        # early completions pile up in the DDict with no monitor watching. On
        # a 10k-task submit that's a multi-second head-of-line block before
        # any callback can fire. Registering inline lets the monitor deliver
        # early results while later tasks are still being translated.
        submitted = 0
        for task in tasks:
            try:
        ...
        self.logger.info(f"Submitted {submitted} tasks (streaming, auto-dispatched)")
...
class DragonExecutionBackendV3Client(BaseBackend):
    ...
    def __init__(
        ...
        # Per-instance coordination dir: multiple clients in the same process
        # must not collide on endpoints.json, so include a uuid suffix (§6.2).
        self._coordination_dir = coordination_dir or os.path.join(
            os.path.expanduser("~"), ".rhapsody", "sessions",
