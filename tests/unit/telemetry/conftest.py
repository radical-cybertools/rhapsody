...
class AdapterCapabilities:
    ...
    Fields that are False/None mean the adapter either cannot collect that
    metric (e.g. Dask has no net I/O) or the hardware is absent.  The
    contract assertions use these flags to distinguish intentional gaps from
    bugs.
    """
    ...
    required_fields: list[str] = field(default_factory=lambda: [
        "event_id", "event_type", "event_time", "emit_time",
        "session_id", "backend",
    ])
